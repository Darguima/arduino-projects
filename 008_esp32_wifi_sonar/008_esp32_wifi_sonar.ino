...
#define DOOR_OPEN_DISTANCE_CM 25
  ...
  while (client.connected() && current_time - previous_time <= TIMEOUT_TIME)
  {
    ...
    if (c == '\n')
    {
      if (current_line.length() == 0)
      {
        ...
        else if (header.indexOf("GET /is_the_door_opened") >= 0)
        {
          Serial.println("[LOG] Responding sensor values");
          server_response(client, 200, "text/plain", is_the_door_opened() ? "1" : "0");
        }
        else
        {
